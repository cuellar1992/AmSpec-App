import api from './api'
import { listMolekulisLoadings, type MolekulisLoading } from './molekulisLoadingService'
...
function hasRestrictedDay(shift: Shift, restrictedDays: number[]): boolean {
  ...
  while (current <= end) {
    if (isRestrictedDay(current, restrictedDays)) {
      return true
    }
...
export async function autogenerateLineSampling(
  ...
  try {
    ...
    const samplers: Sampler[] = samplersResponse.data as any[]
    ...
    auditLog.push(`ERROR: ${error.message}`)
    console.error('Autogenerate error:', error)
    return {
      success: false,
      errors: [`Autogenerate failed: ${error.message}`],
      auditLog
    }
  }
}
